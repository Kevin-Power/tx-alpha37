...
6. Do not treat ORB-15 as a finding. Do not add presets for H-06 skip080-only, H-11 atrExpand, H-04 TX, H-05 autopsy, H-07 MA×weekday, H-02 Friday, H-03 settlement-Monday, H-08 bear-year, or H-09 VWAP×vol. Do not retune `ATR_EXPAND_K`.
7. Reproduce with `npx --yes tsx scripts/run.ts`, `scripts/h06-gap.ts` (TWII), `scripts/h11-atr.ts` (TWII), `scripts/h04-tx.ts`, `scripts/h05-autopsy.ts`, `scripts/h07-ma.ts`, `scripts/h01-real1m.ts`, `scripts/h01-probegrid.ts`, `scripts/h02-friday.ts`, `scripts/h03-settle-mon.ts`, `scripts/h08-bear.ts`, `scripts/h09-vwap.ts`. H-01-30d probe grid and H-09 VWAP×vol are diagnostics, not a pass/fail — do not change `probeMin`, do not turn off `vwapFilter`/`volFilter`. H-03 killed skip-settlement-Monday. H-02 Friday is not adopted. H-08 killed struct37 bear-year bonus — do not change `SAMPLE_START`.
