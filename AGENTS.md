...
6. Do not treat ORB-15 as a finding. Do not add presets for H-06 skip080-only, H-11 atrExpand, H-04 TX, H-05 autopsy, or H-07 MA×weekday. Do not retune `ATR_EXPAND_K`.
7. Reproduce with `npx --yes tsx scripts/run.ts`, `scripts/h06-gap.ts` (TWII), `scripts/h11-atr.ts` (TWII), `scripts/h04-tx.ts`, `scripts/h05-autopsy.ts`, `scripts/h07-ma.ts`, `scripts/h01-real1m.ts`, `scripts/h01-probegrid.ts`. H-01-30d probe grid is a diagnostic, not a pass/fail — do not change `probeMin`.
