...
6. Do not treat ORB-15 as a finding. Do not add presets for H-06 skip080-only, H-11 atrExpand, H-04 TX, H-05 autopsy, H-07 MA×weekday, H-02 Friday, or H-03 settlement-Monday. Do not retune `ATR_EXPAND_K`.
7. Reproduce with `npx --yes tsx scripts/run.ts`, `scripts/h06-gap.ts` (TWII), `scripts/h11-atr.ts` (TWII), `scripts/h04-tx.ts`, `scripts/h05-autopsy.ts`, `scripts/h07-ma.ts`, `scripts/h01-real1m.ts`, `scripts/h01-probegrid.ts`, `scripts/h02-friday.ts`, `scripts/h03-settle-mon.ts`. H-01-30d probe grid is a diagnostic, not a pass/fail — do not change `probeMin`. H-03 killed skip-settlement-Monday. H-02 Friday is not adopted.
