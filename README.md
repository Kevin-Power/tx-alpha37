...
**假 edge 殺得差不多了，策略還沒成形。** H-04 把日線換成台指期真開盤價之後，H-06 的缺口放假通過條件失敗（TX 上無缺口 PF 1.08，只放假 0.8 變成 1.04）；結構37 沒加分。加權路徑上唯一種子穩健的結論仍是負結論：無濾網 TORB 扣完成本後沒有 edge。下表的正 PF 是 seed 0 加權數字，只能當上界。這不是投資建議。
...
被推翻：跳過月結算週三（結算當天 18 筆 PF 3.19）、外資水位濾網（37X 更差）、把 0.55 缺口順勢當 A 層升級（H-06）、把結構37 解釋成 ATR 擴張 2.0（H-11，0 日開火）、把 ≥ 0.8 ATR 放假當已確認 A 層法則（H-04：只在加權假開盤上通過）。真正差的是**結算週的週一**（19 筆 PF 0.17）。沒有新預設。
...
```
RESEARCH.md          完整方法、結果、限制、文獻（§5.6 = H-06，§5.7 = H-11，§5.8 = H-04）
FOR_MODELS.md        給其他 LLM 的研究協議（必讀）
OPEN_QUESTIONS.md    下一步要驗的假設
src/                 可重跑的 TypeScript 引擎（預設 MARKETS.tx）
results/             2026-08-25 的 KPI dump（含 h06-gap.json、h11-atr.json、h04-tx.json）
scripts/run.ts       重跑預設組合（TX）
scripts/h06-gap.ts   重跑缺口拆層（加權重現）
scripts/h11-atr.ts   重跑 ATR 擴張放假（K=2.0，加權重現）
scripts/h04-tx.ts    TX 真開盤價重驗 H-06／preset／種子
```
