...
ALPHA-37 **不是印鈔機**。全樣本獲利因子 1.15、樣本外 1.19；2025 年接近打平，2026 年多頭結構才拉起來。日線濾網（20 日均、避開週一／週三、週五選擇權結構）比把探針調成 15 分更可信——因為 1 分 K 是日 OHLC **重建**，不是期交所逐筆。
...
被推翻：跳過月結算週三（結算當天 18 筆 PF 3.19）、外資水位濾網（37X 更差）。真正差的是**結算週的週一**（19 筆 PF 0.17）。

## 倉庫結構

```
RESEARCH.md          完整方法、結果、限制、文獻
