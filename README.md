...
**目前唯一種子穩健的結論是負結論：無濾網 TORB 扣完成本後沒有 edge（0/20 條重建路徑 PF>1）。** 下表的正 PF 全部來自單一條重建路徑（seed 0），而該路徑已被證實在 60 條路徑中排 98–100 百分位；種子中位數下 ALPHA-37 全樣本是虧的。日線濾網裡目前站得住的：H-06 把舊「缺口濾網」拆開後，≥ 0.8 ATR 整日放假（`gapSkip080`）有條件通過 A 層 ablation；0.55 順勢幾乎沒加分，不列升級。結構37 數字漂亮但增益 84% 集中在週四／週五，且 MA 單調性測試失敗，降級為體制依賴。詳見 RESEARCH.md §5.6 與第 10 節。
